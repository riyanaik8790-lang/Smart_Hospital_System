...
function MetricCard({ metric }) {
  ...
  return <article className={`min-h-[132px] rounded-lg border p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md md:p-6 ${metric.cardClass}`}><div className="flex h-full items-center justify-between gap-5"><div><p className="text-xs font-medium leading-none text-slate-700">{metric.label}</p><p className="mt-2 text-4xl font-bold leading-none tracking-tight text-slate-950">{metric.value}</p><p className="mt-2 text-[11px] leading-none text-slate-600">{metric.detail}</p></div><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-lg ${metric.iconClass}`}><Icon size={20} strokeWidth={1.8} /></span></div></article>;
}
...
export default function DashboardPage() {
  ...
  return <main className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-1 py-1 sm:gap-5">
    ...
    <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 lg:grid-cols-4" aria-label="Hospital metrics">{metrics.map((metric) => <MetricCard key={metric.label} metric={metric} />)}</section>
