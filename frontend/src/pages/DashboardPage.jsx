...
function MetricCard({ metric }) {
  ...
  return <article className={`min-h-[168px] rounded-xl border p-7 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md md:p-8 ${metric.cardClass}`}><div className="flex h-full items-start justify-between gap-6"><div><p className="text-sm font-semibold leading-none text-slate-700">{metric.label}</p><p className="mt-4 text-5xl font-extrabold leading-none tracking-tight text-slate-950">{metric.value}</p><p className="mt-3 text-xs leading-none text-slate-600">{metric.detail}</p></div><span className={`grid h-14 w-14 shrink-0 place-items-center rounded-xl ${metric.iconClass}`}><Icon size={26} strokeWidth={1.9} /></span></div></article>;
}
...
export default function DashboardPage() {
  ...
  return <main className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-1 py-1 sm:gap-5">
    ...
    <section className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:gap-7 lg:grid-cols-4" aria-label="Hospital metrics">{metrics.map((metric) => <MetricCard key={metric.label} metric={metric} />)}</section>
