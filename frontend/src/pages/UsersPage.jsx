...
const UsersPage = () => {
  ...
  return <div className="page-container"><div className="page-header"><div><h1 className="page-title">User management</h1><p className="page-subtitle">Create staff accounts, manage roles, and deactivate access while retaining clinical history.</p></div><button className="btn btn-primary" onClick={() => { setError(''); setCreateOpen(true); }}><Plus size={18} /> Create Staff Account</button></div>{message && <div className="alert alert-success">{message}</div>}{error && <div className="alert alert-error">{error}</div>}<div className="section-card"><div className="table-container"><table className="table"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Action</th></tr></thead><tbody>{users.map((u) => <tr key={u.user_id} id={`user-${u.user_id}`} className={highlightedUserId === u.user_id ? 'password-reset-requested-user' : ''}><td><strong>{u.name}</strong></td><td>{u.email}</td><td><select className="form-control" value={u.role} disabled={savingRoleId === u.user_id} onChange={(event) => changeRole(u, event.target.value)} aria-label={`Change ${u.name}'s role`} style={{ minWidth: 130, textTransform: 'capitalize' }}>{['admin', 'doctor', 'nurse', 'receptionist'].map((role) => <option key={role} value={role}>{role[0].toUpperCase() + role.slice(1)}</option>)}</select></td><td><span className={`badge ${u.is_active ? 'badge-low' : 'badge-high'}`}>{u.is_active ? (u.must_change_password ? 'Password setup required' : 'Active') : 'Deactivated'}</span></td><td>{u.is_active && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}><button className="btn btn-outline" style={{ padding: '6px 10px', fontSize: 12 }} onClick={() => { setMessage(''); setPasswordOverrideUser(u); }}><KeyRound size={14} /> Change Password</button><button className="btn btn-danger" style={{ padding: '6px 10px', fontSize: 12 }} onClick={() => { setMessage(''); setSelected(u); }}><UserX size={14} /> Deactivate</button></div>}</td></tr>)}{!users.length && <tr><td colSpan="5" className="text-center">No users found.</td></tr>}</tbody></table></div></div>{selected && <DeactivateAccountModal user={selected} onClose={() => setSelected(null)} onConfirm={deactivate} />}{passwordOverrideUser && <PasswordOverrideModal user={passwordOverrideUser} onClose={() => setPasswordOverrideUser(null)} onConfirm={forceReset} />}{createOpen && <CreateStaffAccountModal onClose={() => setCreateOpen(false)} onCreate={createStaff} />}</div>;
};
